  ...
  onChangeEmpresa(data) {
    ...
    this.listaEmpresa.forEach(element => {
      if (element.id_empresa == data) {
        ...
        if(this.tipoempresaselected == "2") {
          this.emprSer.listarSubrogados(element.id_empresa).subscribe(response => {
            console.log(response)
    ...
    if(this.bancayseguros){
      this.loadServ.listarEmpresa(this.tipoIdentificacion, this.nroIdentificacion, this.codigoEmpresa,
        this.razonSocial, "01", this.grupo_vendedor, this.numero_convenio).subscribe(response => {
          ...
          var grupoVendedorSW = conveniosList[0]["DatosCabecera"]["GrupoVendedor"];
          console.log(grupoVendedorSW);
          this.convenioRolRecaudador = (conveniosList[0]["DatosCabecera"]["Convenio"] + " " + result["DatosGenerales"]["RazonSocial"]+ " " + conveniosList[0]["DatosCabecera"]["DescripcionGpoVendedor"])

            // no tiene Broker, ejecuta vendedor
            this.codBpVendedor = conveniosList[0]["UnidadVenta"][0]["CodigoBPUnidadVenta"];
            this.loadServ.listarEmpresa(this.tipoIdentificacion,'',this.codBpVendedor,this.razonSocial,'00',this.grupo_vendedor,this.numero_convenio)
              .subscribe((responseDos) => {
                ...
                if(grupoVendedorSW == conveniosListTwo[0]["DatosCabecera"]["GrupoVendedor"]){
                  this.convenioRolUnidVenta =
