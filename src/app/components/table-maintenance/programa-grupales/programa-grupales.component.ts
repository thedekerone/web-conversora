...
export class ProgramaGrupalesComponent implements OnInit {
  ...
  constructor(
    private pgServ: ProgramaGrupalesService,
  ...
  listarPGs() {
    this.pgServ.listarPGs().subscribe((response) => {
      ...
      this.listaPGrupales.sort((a, b) => b.estado - a.estado);
  ...
  abrirModalActualizar(id_programa_grupales) {
    ...
    this.pgServ.editarPG(id_programa_grupales).subscribe((response) => {
      ...
      this.updatePGForm.controls.programa_trama.setValue(
        listaPGrupal.programa_trama
      );
      this.updatePGForm.controls.cod_programa_sap.setValue(
        listaPGrupal.cod_programa_sap
      );
    });
  }
  ...
  abrirModalDesactivar(id_programa_grupales) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.pgServ
          .desactivarPG(id_programa_grupales, 0)
          .subscribe((response) => {
  ...
  abrirModalActivar(id_programa_grupales) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.pgServ
          .desactivarPG(id_programa_grupales, 1)
          .subscribe((response) => {
            Swal.fire('Estado actualizado', '', 'success');
            this.dtTrigger.unsubscribe();
            this.listarPGs();
