...
export class PlanCampanaComponent implements OnInit {
  ...
  constructor( private plancampanaServ: PlanCampanaService, private utilsDt: DatatableService, private formBuilder: FormBuilder) {
    this.listaPlanes = [];
  ...
  abrirModalActualizar(procedencia, cod_empresa){
    ...
    this.plancampanaServ.editarCampanas(procedencia, cod_empresa).subscribe(response => {
  ...
  btnRegistrar() {
    ...
    this.plancampanaServ.crearCampanas(this.registerPYCForm.value).subscribe(response => {
  ...
  btnActualizar() {
    ...
    this.plancampanaServ.actualizarCampanas(this.updatePYCForm.value).subscribe(response => {
      if(response['success'] == true){
        Swal.fire({
          icon: 'success',
          title: 'Proceso completado',
  ...
  abrirModalDesactivar(procedencia, cod_empresa) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.plancampanaServ.desactivarCampanas(procedencia, cod_empresa, 0).subscribe(response => {
  ...
  abrirModalActivar(procedencia, cod_empresa) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.plancampanaServ.desactivarCampanas(procedencia, cod_empresa, 1).subscribe(response => {
          Swal.fire(response['message'], '', 'success')
