...
export class ProgramaComponent implements OnInit {
  ...
  constructor(
    private programServ: ProgramaService,
  ...
  listarProgramas() {
    this.programServ.listarProgramas().subscribe((response) => {
      ...
      console.log(this.listaPrograms);
      this.listaPrograms.sort((a, b) => b.estado - a.estado);
      this.dtTrigger.next();
    });
  }
  ...
  abrirModalActualizar(cod) {
    ...
    this.programServ.editarPrograma(cod).subscribe((response) => {
      ...
      this.updateProgramForm.controls.cod_programa_oncosys.setValue(
        listaPrograma.cod_programa_oncosys
      );
      this.updateProgramForm.controls.descripcion.setValue(
        listaPrograma.descripcion
      );
      this.updateProgramForm.controls.cod_programa_sap.setValue(
        listaPrograma.cod_programa_sap
      );
    });
  }
  ...
  btnRegistrar() {
    this.programServ
      .crearPrograma(this.registerProgramForm.value)
      .subscribe((response) => {
  ...
  btnActualizar() {
    this.programServ
      .actualizarPrograma(this.updateProgramForm.value)
      .subscribe((response) => {
        if (response['success'] == true) {
          Swal.fire({
            icon: 'success',
  ...
  abrirModalDesactivar(cod_program) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.programServ
          .desactivarPrograma(cod_program, 0)
          .subscribe((response) => {
  ...
  abrirModalActivar(cod_program) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.programServ
          .desactivarPrograma(cod_program, 1)
          .subscribe((response) => {
            Swal.fire(response['message'], '', 'success');
            this.dtTrigger.unsubscribe();
            this.listarProgramas();
