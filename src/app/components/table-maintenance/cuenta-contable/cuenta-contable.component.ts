...
export class CuentaContableComponent implements OnInit {
  ...
  constructor( private cuentacontaServ: CuentaContableService, private utilsDt: DatatableService, private formBuilder: FormBuilder) {
    this.listaContables = [];
  ...
  abrirModalActualizar(id_cuenta_contable){
    ...
    this.cuentacontaServ.editarContable(id_cuenta_contable).subscribe(response => {
      ...
      this.updateContableForm.controls.id_cuenta_contable.setValue(id_cuenta_contable);

      this.updateContableForm.controls.origen.setValue(listaContable.origen);
      this.updateContableForm.controls.cuenta_contable.setValue(listaContable.cuenta_contable);
      this.updateContableForm.controls.segmento.setValue(listaContable.segmento);
  ...
  btnRegistrar() {
    ...
    this.cuentacontaServ.crearContable(this.registerContableForm.value).subscribe(response => {
  ...
  btnActualizar() {
    ...
    this.cuentacontaServ.actualizarContable(this.updateContableForm.value).subscribe(response => {
      if(response['success'] == true){
        Swal.fire({
          icon: 'success',
          title: 'Proceso completado',
  ...
  abrirModalDesactivar(id_cuenta_contable) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.cuentacontaServ.desactivarContable(id_cuenta_contable, 0).subscribe(response => {
  ...
  abrirModalActivar(id_cuenta_contable) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.cuentacontaServ.desactivarContable(id_cuenta_contable ,1).subscribe(response => {
          Swal.fire(response['message'], '', 'success')
