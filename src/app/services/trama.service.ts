...
export class TramaService {
  ...
  listarProductos() {
    ...
    return this.http
      .post<Object>(environment.baseRoutePath, params)
      .pipe(map((res) => res));
  }

  ASD() {
    return new Promise((resolve, reject) => {
      this.http.get(this.apiService._END_POINTS.URL_GET_USER_PROFILE).subscribe(
        (success) => {
          resolve(success);
        },
        (error) => {
  ...
  cargarArchivoPago(extension, file, tipo_archivo, nombre_archivo) {
    ...
    return this.http
      .post<Object>(
        this.apiService._END_POINTS.BASE_ROUTE_PATH_UPLOAD_ARCHIVO_PAGO,
        params
      )
      .pipe(map((res) => res));
  }
  ...
  registrarTramaTmk(
    ...
    console.log(params);
    return this.http
      .post<Object>(this.apiService._END_POINTS.BASE_ROUTE_PATH_SAVE, params)
      .pipe(map((res) => res));
  }
  ...
  revisarEstadoProceso(codProceso) {
    ...
    return this.http
      .post<Object>(
        this.apiService._END_POINTS.BASE_ROUTE_PATH_ESTADO_CARGA_PROCESOS,
        params
      )
      .pipe(map((res) => res));
  }
  ...
  registrarTramaByS(
    ...
    capitalizado,
    cod_bp_vendedor,
    cod_convenio_bp_vendedor,
    cod_bp_broker,
    cod_convenio_broker,
    cod_bp_empresa_recaudador,
    cod_convenio_recaudador,
    canal_venta
  ) {
    const params = {
      action: 'carga_trama',
      ...
      // cod_bp_broker: cod_bp_broker ? cod_bp_broker + '' : 'vacio',
  ...
  registrarTramaGrup1(
    ...
    const params = {
      ...
      nombre_archivo: nombre_archivo,
      file: file,
      canal: canal ? canal + '' : 'vacio',
      id_canal: id_canal ? id_canal + '' : 'vacio',
      tipo_trama: tipo_trama ? tipo_trama + '' : 'vacio',
      empresa_bp: empresa_bp ? empresa_bp + '' : 'vacio',
      id_usuario: id_usuario ? id_usuario + '' : 'vacio',
      id_tipo_trama: id_tipo_trama ? id_tipo_trama + '' : 'vacio',
      id_empresa: id_empresa ? id_empresa + '' : 'vacio',
      sede_carga: sede_carga ? sede_carga + '' : 'vacio',
  ...
  registrarTramaGrup2(
    ...
    const params = {
      ...
      cod_convenio_bp_vendedor: cod_convenio_bp_vendedor,
      cod_bp_broker: cod_bp_broker ? cod_bp_broker : 'vacio',
      cod_convenio_broker: cod_convenio_broker ? cod_convenio_broker : 'vacio',
      cod_bp_sede: codigoBPSede ? codigoBPSede + '' : 'vacio',
      canal_venta: canal_venta,
      form_plan: form_plan ? form_plan : 'vacio',
      tipo_via: tipo_via,
      nombre_via: nombre_via,
      nombre_via2: nombre_via2,
      nombre_via3: nombre_via3,
      campo_manual: campo_manual,
      form_manzana: form_manzana ? form_manzana : 'vacio',
      form_lote: form_lote ? form_lote : 'vacio',
      form_dpt: form_dpt ? form_dpt : 'vacio',
      form_sector: form_sector ? form_sector : 'vacio',
